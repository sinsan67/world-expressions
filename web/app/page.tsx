...
export default function Home() {
  ...
  const [exploreCountryOpen, setExploreCountryOpen] = useState(true);
  const [exploreConceptOpen, setExploreConceptOpen] = useState(true);
    ...
    <main className="min-h-screen" style={{ background: "#f5f3ff" }}>
          ...
          >
            ...
            {!searched && featured && (
              ...
              >
                ...
                {/* Chips suggestion sous l'encart */}
                <div style={{ marginTop: "0.6rem", display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                  <button
                    onClick={() => router.push(`/country/${featured.region}`)}
                    style={{
                  ...
                  >
                    {FLAG[featured.region] ?? "🌍"} {COUNTRY_NAME[featured.region] ?? featured.region.toUpperCase()}
                  </button>
                  {featured.tags.slice(0, 2).map((tag) => {
                    ...
                    return (
                      <button
                        key={tag}
                        onClick={() => { setSearchLabel(`${icon} ${name}`); runConceptSearch(tag, activeRegions); }}
                        style={{
                          fontSize: 12, padding: "4px 12px", borderRadius: 20,
      ...
      <div ref={exploreRef} style={{ background: "#f5f3ff", padding: "2rem 1rem 1.5rem", borderBottom: "1px solid #ede9fe" }}>
        <div style={{ maxWidth: 620, margin: "0 auto", textAlign: "center" }}>
          ...
          {/* Explorer un pays + Explorer un concept */}
          <div style={{ display: "flex", gap: "2rem", marginBottom: "1rem", flexWrap: "wrap" }}>

            {/* Explorer un pays */}
            <div style={{ flex: 1, minWidth: 180 }}>
              <button
                onClick={() => setExploreCountryOpen((v) => !v)}
                style={{
              ...
              >
                🌍 {t.exploreCountry}
                <span style={{ fontSize: 8 }}>{exploreCountryOpen ? "▴" : "▾"}</span>
              </button>
              {exploreCountryOpen && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
                  {regions.map((r) => (
                    <button
                      ...
                      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "#f5f3ff"; (e.currentTarget as HTMLElement).style.borderColor = "#c4b5fd"; }}
            ...
            <div style={{ flex: 1, minWidth: 180 }}>
              <button
                onClick={() => setExploreConceptOpen((v) => !v)}
                style={{
                  display: "flex", alignItems: "center", gap: "0.35rem",
                  background: "none", border: "none", cursor: "pointer", padding: 0,
                  fontSize: 11, fontWeight: 700, textTransform: "uppercase",
              ...
              >
                🔍 {t.exploreConcept}
                <span style={{ fontSize: 8 }}>{exploreConceptOpen ? "▴" : "▾"}</span>
              </button>
              {exploreConceptOpen && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
                  {hintTags.map((tag) => {
